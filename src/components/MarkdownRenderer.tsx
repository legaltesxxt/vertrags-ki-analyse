...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content }) => {
  ...
  const components = {
    h2: ({ node, ...props }: any) => (
      <h2 className="text-2xl font-light tracking-tight text-legal-primary mt-10 mb-4 pb-2 border-b border-legal-tertiary/60" {...props} />
    ),
    h3: ({ node, ...props }: any) => (
      <h3 className="text-xl font-medium text-legal-secondary mt-8 mb-3" {...props} />
    ),
    h4: ({ node, ...props }: any) => (
      <h4 className="text-lg font-medium text-legal-primary/80 mt-6 mb-2" {...props} />
    ),
    p: ({ node, children, ...props }: any) => {
      ...
      if (text.includes('rechtlich unzulässig') || text.includes('hohes risiko') || text.includes('problematisch')) {
        return (
          <div className="flex items-start gap-3 p-4 bg-red-50 text-red-800 rounded-lg my-4 border border-legal-risk-high/30" {...props}>
        ...
        return (
          <div className="flex items-start gap-3 p-4 bg-orange-50 text-orange-800 rounded-lg my-4 border border-legal-risk-medium/30" {...props}>
        ...
        return (
          <div className="flex items-start gap-3 p-4 bg-green-50 text-green-800 rounded-lg my-4 border border-legal-risk-low/30" {...props}>
        ...
        return (
          <div className="flex items-start gap-3 p-4 bg-blue-50 text-blue-800 rounded-lg my-4 border border-blue-300/30" {...props}>
    ...
    ul: ({ node, ...props }: any) => (
      <ul className="list-disc pl-6 my-4 space-y-2 text-slate-700 text-sm" {...props} />
    ),
    ol: ({ node, ...props }: any) => (
      <ol className="list-decimal pl-6 my-4 space-y-2 text-slate-700 text-sm" {...props} />
    ),
    li: ({ node, ...props }: any) => (
      <li className="my-1 text-sm" {...props} />
    ),
    a: ({ node, ...props }: any) => (
      <a className="text-legal-secondary hover:text-legal-primary hover:underline font-medium transition-colors" {...props} target="_blank" rel="noopener noreferrer" />
    ),
    hr: ({ node, ...props }: any) => (
      <hr className="my-8 border-gray-200" {...props} />
    ),
    blockquote: ({ node, ...props }: any) => (
      <blockquote className="border-l-4 border-legal-primary pl-4 italic my-6 text-slate-600 bg-legal-tertiary/30 p-3 rounded-r text-sm" {...props} />
    ),
    code: ({ node, inline, ...props }: any) => (
      inline 
        ? <code className="bg-slate-100 px-1.5 py-0.5 rounded text-xs font-mono" {...props} />
        : <code className="block bg-slate-100 p-4 rounded-md my-4 text-xs font-mono overflow-x-auto" {...props} />
    ),
